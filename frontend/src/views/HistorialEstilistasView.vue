<template>
  <section class="hist-page">
    ...
    <div class="page-header">
      <div>
        <h1>Historial por estilista</h1>
        <p>Consulta los servicios realizados y el reparto por arrendamiento de cada estilista.</p>
      </div>
    ...
    <div class="filtros-card">
      <div class="filtro">
        ...
        <select v-model="idEmpleado" @change="cargar">
    ...
    <!-- ═══ RESUMEN ═══ -->
    <div v-if="idEmpleado" class="resumen-grid">
      ...
      <div class="resumen-card destacada">
        <span class="rc-label">A pagar al estilista</span>
    ...
    <div class="seccion-card">
      <div v-if="!idEmpleado" class="vacio">Selecciona un estilista para ver su historial.</div>
      <div v-else-if="filas.length === 0" class="vacio">
...
<script>
import { getHistorialEstilista } from '@/services/historialEstilistaService'
import { getEmpleados } from '@/services/empleadoService'
import AppToast from '@/components/AppToast.vue'
import { historialPdf } from '@/utils/historialPdf'

export default {
  name: 'HistorialEstilistasView',
  ...
  methods: {
    ...
    async cargarEstilistas() {
      try {
        ...
        this.estilistas = res.data.filter(e => e.usuario?.rol?.nombreRol === 'EMPLEADO')
      } catch (e) {
        console.error(e)
        this.mostrarToast('error', 'Error', 'No se pudieron cargar los estilistas.')
      }
    },
    ...
    async cargar() {
      ...
      try {
        const res = await getHistorialEstilista(this.idEmpleado, this.desde, this.hasta)
...
.filtro select:focus, .filtro input:focus {
  border-color: #014421; box-shadow: 0 0 0 4px rgba(1, 68, 33, 0.08);
}
